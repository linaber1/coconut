...
#torchrun --standalone --nproc_per_node=2 train_stokenizer.py --compile=False --batch_size=16
...
parser.add_argument('--n_head', type=int, default=4, help='Number of attention heads (default: 1)')  
...
_meta_block_size = 200 
n_layer = 2 #args.n_layer
n_head = 8 #args.n_head
n_embd = 768 #args.n_embd
...
# I/O
eval_interval = max(1, max_iters // 500)
...
min_lr = learning_rate# CONSTANT LEARNING RATE /10 # minimum learning rate, should be ~= learning_rate/10 per Chinchilla
# DDP settings
...
def get_batch(split):
    ...
    for idx in indices:
        ...
        for _ in range(path_nb_pergraph): # add multiple paths per graph
            ...
            y[:question_len-1] = -100 #CAREFUL WHEN YOU SHIFT
